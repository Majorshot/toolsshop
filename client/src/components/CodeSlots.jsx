...
.cs-slot {
  ...
  background-color: var(--cs-slot);
  user-select: none;
  transition: background-color 200ms ease, box-shadow 200ms ease, border-color 200ms ease;
  box-sizing: border-box;
}
.cs-slot[data-active] {
  background-color: color-mix(in srgb, var(--cs-ink) 12%, var(--cs-slot));
}
.cs-row[data-status="error"] .cs-slot {
  background-color: color-mix(in srgb, var(--cs-danger) 20%, var(--cs-slot));
}
.cs-slot-fill {
  position: absolute;
  inset: 0;
...
.cs-row[data-status="error"] .cs-slot-fill {
    ...
    >
      ...
      >
        ...
        >
          <span
            key={active}
            className="cs-caret-inner block h-full w-full bg-[var(--cs-ink)] animate-[code-slots-blink_1s_linear_infinite] motion-reduce:animate-none"
          />
        </motion.span>
      </div>
      <span id={`${uid}-count`} className="cs-sr-only sr-only" aria-live="polite">
