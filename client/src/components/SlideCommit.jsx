...
.sc-root[data-phase="pending"] .sc-track,
.sc-root[data-phase="done"] .sc-track {
  cursor: default;
}
.sc-track-label {
  pointer-events: none;
...
.sc-label-error {
  color: var(--sc-no) !important;
  opacity: 0;
  filter: blur(2px);
}
.sc-root[data-phase="error"] .sc-label-error {
  opacity: 1;
  filter: blur(0);
}
.sc-capsule {
...
.sc-root[data-phase="pending"] .sc-arrow {
  filter: blur(2px);
}
.sc-spinner-wrap {
  filter: blur(2px);
}
.sc-root[data-phase="pending"] .sc-spinner-wrap {
  filter: blur(0);
}
.sc-sr-only {
  position: absolute;
...
export default function SlideCommit({
  ...
  const [realWidth, setRealWidth] = useState(typeof width === 'number' ? width : 320);
  ...
  const effectiveWidth = typeof width === 'number' ? width : realWidth;
  const GRIP = height - PAD * 2;
  const INNER = Math.max(GRIP + 2, effectiveWidth - PAD * 2);
  ...
  return (
    ...
    >
      ...
      <motion.div
        ref={trackRef}
        className="sc-track relative h-full w-full cursor-grab touch-none select-none [-webkit-touch-callout:none] [-webkit-tap-highlight-color:transparent] rounded-[var(--sc-radius)] [background:var(--sc-track)] group-data-[held]:cursor-grabbing group-data-[phase=pending]:cursor-default group-data-[phase=done]:cursor-default"
      ...
      >
        ...
        <span className="sc-sr-only sr-only" aria-live="polite">
