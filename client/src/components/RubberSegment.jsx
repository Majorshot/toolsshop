...
const RUBBER = 0.55;
const SIZES = {
  sm: { height: 30, font: 12, pad: 10, min: 36 },
...
.rs-btn[aria-checked="true"] {
  cursor: default;
}
.rs-track[data-draggable] .rs-btn[aria-checked="true"] {
  cursor: grab;
}
.rs-track[data-held] .rs-btn {
  cursor: grabbing !important;
}
.rs-btn[data-pressed] {
  transform: scale(0.96);
}
.rs-btn[aria-checked="false"]:hover {
  opacity: 0.95;
}
.rs-thumb-overlay {
  pointer-events: none;
...
.rs-track[data-equal] .rs-thumb-overlay {
  grid-auto-columns: minmax(0, 1fr);
}
.rs-thumb-label {
  display: inline-flex;
...
export default function RubberSegment({
  ...
  useEffect(() => {
    if (!drag.current && committed.current !== index) {
      ...
      if (slots.current.length && slots.current[from] && slots.current[index]) {
        travel(from, index);
      } else {
        jumpTo(index);
      }
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="rs-thumb-overlay pointer-events-none absolute inset-[var(--rs-inset)] grid grid-flow-col auto-cols-auto group-data-[equal]:auto-cols-[minmax(0,1fr)] [background:var(--rs-thumb)] [color:var(--rs-ink-active)]"
        aria-hidden="true"
        style={{ clipPath }}
      >
        {list.map(item => (
          <span
            key={item.value}
            className="rs-thumb-label inline-flex h-[calc(var(--rs-h)-var(--rs-inset)*2)] min-w-[var(--rs-min)] items-center justify-center gap-1.5 m-0 border-0 bg-transparent px-[var(--rs-pad)] py-0 rounded-[var(--rs-thumb-radius)] [font:inherit] text-[length:var(--rs-font)] font-medium leading-none whitespace-nowrap outline-none [transition:opacity_160ms_ease,transform_160ms_var(--rs-ease-out)] motion-reduce:[transition:opacity_160ms_ease] cursor-default [color:inherit]"
          >
            {item.icon}
            {item.label}
          </span>
